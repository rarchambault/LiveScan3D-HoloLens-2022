Shader "Custom/ParticleBillboardFade"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Compute distance from center of the quad (0.5, 0.5 in UV space)
                float dist = length(i.uv - float2(0.5, 0.5)); 

                // Smooth fade that extends further towards the edges
                float fade = smoothstep(0.2, 0.55, dist); // Starts earlier, fades slower

                // Apply fade effect to alpha
                col.a *= (1.0 - fade); // Invert fade so center is fully visible
